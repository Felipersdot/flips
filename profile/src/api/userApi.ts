...
if (process.env.REACT_APP_USER_REGISTRATION_PATH && process.env.NODE_ENV !== 'production') {
...
const auth = {
    ...
    login: (email: string, password: string) => {
        ...
        return Transport.post(`${USER_API}/${AUTH_END_POINT}/login`, userCredentials);
    }
};
...
const userRegistrationToken = {
    sendInvitation: (email: string, roleId: string, companyId: string, buidlerSubdomain: string | null): AxiosPromise<string>  => {
        const data = {
            email,
            roleId,
            companyId
